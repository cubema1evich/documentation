...
Модуль auth.py отвечает за регистрацию, вход, выход из системы и удаление сессий. Именно он запускает пользователя в систему и следит за тем, чтобы он был авторизован. Ниже представлена таблица \ref{tab:auth_classes} с описанием классов в модуле auth.py 
...
Модуль base.py содержит базовые абстракции (View, TemplateView), от которых наследуются другие обработчики, а также класс IndexView, рендерящий главную страницу. Ниже представлена таблица \ref{tab:base_classes} с описанием классов в модуле base.py
...
Модуль message.py - главный мотор общения: получение, отправка, редактирование, удаление сообщений, работа с вложениями, системными событиями, проверками изменений. Ниже представлена таблица \ref{tab:message_classes} с описанием классов в модуле message.py
...
Модуль p\_chat.py обслуживает личные диалоги один-на-один. Ниже представлена таблица \ref{tab:p_chat_classes} с описанием классов в модуле p\_chat.py
...
Модуль search.py отвечает за поиск пользователей и сообщений. Ниже представлена таблица \ref{tab:search_classes} с описанием классов в модуле search.py
...
Модуль users.py работает с данными пользователей и их ролями в групповых чатах. Ниже представлена таблица \ref{tab:users_classes} с описанием классов в модуле users.py
...
Модуль error.py перехватывает и визуально обрабатывает критические ситуации. Ниже представлена таблица \ref{tab:error_classes} с описанием классов в модуле error.py
...
Модуль groups.py управляет всем, что касается групповых чатов. Ниже представлена таблица \ref{tab:groups_classes} с описанием классов в модуле groups.py
...
В таблице \ref{auth-tests} представлены тестовые случаи для модуля аутентификации и авторизации. Проверяет регистрацию новых пользователей, вход в систему, доступ к защищённым ресурсам и обработку ошибок валидации.
...
В таблице \ref{error-tests} представлены тестовые случаи для обработки системных ошибок и некорректных запросов. Проверяет возврат корректных HTTP-статусов для различных ошибочных сценариев.
...
В таблице \ref{group-tests} представлены тестовые случаи для функционала управления группами. Проверяет создание групп, управление участниками, отправку сообщений в группы и контроль доступа.
...
В таблице \ref{messages-tests} представлены тестовые случаи для функционала работы с сообщениями. Проверяет отправку, получение, редактирование и удаление сообщений в различных контекстах.
...
В таблице \ref{private-tests} представлены тестовые случаи для функционала приватных сообщений. Проверяет создание приватных чатов, обмен сообщениями и контроль доступа.
...
В таблице \ref{search-tests} представлены тестовые случаи для функционала поиска. Проверяет поиск пользователей и сообщений по различным критериям с поддержкой фильтрации и пагинации.
